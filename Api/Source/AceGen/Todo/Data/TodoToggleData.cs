/********************************************************************************
 * generated by de.acegen
 ********************************************************************************/




namespace AceGen.Todo.Data;

public class TodoToggleData : AbstractTodoToggleData, ITodoToggleData {
	
	public TodoToggleData(
		string? id, 
		bool? done, 
		AceGen.Todo.Models.ITodoModel? todoToBeToggled, 
		DateTime? updatedDateTime, 
		string uuid
	) : base(id, done, todoToBeToggled, updatedDateTime, uuid) {
	}

	public TodoToggleData( string uuid ) : base(uuid) {
	}

	override public void MigrateLegacyData(String json) {
	}

	public static ITodoToggleData GenerateTestData() {
		Random random = new Random();
		ITodoToggleData testData = (ITodoToggleData) new AceGen.Todo.Models.TodoToggleModel();
		testData.Id = RandomString(random);
		testData.Done = RandomBool(random);
		testData.TodoToBeToggled = AceGen.Todo.Data.TodoData.GenerateTestData();
		testData.UpdatedDateTime = RandomBool(random) ? DateTime.Now.AddMinutes(random.Next(60)) : DateTime.Now.AddMinutes(-1*random.Next(60)) ;;
		return testData;
	}
	
	private static String RandomString(Random random) {
		String chars = "aaaaaaabcdeeeeeeeffffghiiiiiiijkllllllmmmmnnnnnnnooooooooopqrstttuuuuuuuvxyz";
		int length = random.Next(20) + 5;
		return new string(Enumerable.Repeat(chars, length).Select(s => s[random.Next(s.Length)]).ToArray());
	}
	
	private static bool RandomBool(Random random) {
		return random.Next() > (Int32.MaxValue / 2);
	}
	
	private static float RandomFloat(Random random) {
	    double mantissa = (random.NextDouble() * 2.0) - 1.0;
	    double exponent = Math.Pow(2.0, random.Next(-126, 128));
	    return (float)(mantissa * exponent);
	}
	
	
}



/******* S.D.G. *******/



